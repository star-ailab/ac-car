...
class GenerationMixin:
    ...
    #Mine
    def _get_logits_warper2(
        self, top_k: int = None, top_p: float = None, temperature: float = None, num_beams: int = None, extra_sampled_actions: List[int] = None
    ) -> LogitsProcessorList:
        """
        This class returns a :obj:`~transformers.LogitsProcessorList` list object that contains all relevant
        :obj:`~transformers.LogitsWarper` instances used for multinomial sampling.
        ...
        if temperature is not None and temperature != 1.0:
            warpers.append(TemperatureLogitsWarper(temperature))
        if top_k is not None and top_k != 0:
            warpers.append(TopKLogitsWarper_Augmented(top_k=top_k, min_tokens_to_keep=(2 if num_beams > 1 else 1), extra_sampled_actions=extra_sampled_actions))
        if top_p is not None and top_p < 1.0:
            warpers.append(TopPLogitsWarper(top_p=top_p, min_tokens_to_keep=(2 if num_beams > 1 else 1)))
        return warpers
    ...
    def generate2(
        ...
        use_cache: Optional[bool] = None,
        prefix_allowed_tokens_fn: Optional[Callable[[int, torch.Tensor], List[int]]] = None,
        extra_sampled_actions: Optional[List[int]] = None,
        **model_kwargs
    ) -> torch.LongTensor:
        r"""
        Generates sequences for models with a language modeling head. The method currently supports greedy decoding,
        ...
        if is_greedy_gen_mode:
            ...
            # get probability distribution warper
            logits_warper = self._get_logits_warper2(
                top_k=top_k, top_p=top_p, temperature=temperature, num_beams=num_beams, extra_sampled_actions = extra_sampled_actions
            )
