...
class embed_ActorCritic(Agent):
    ...
    def get_action(self, state, explore=0):
        ...
        if self.initial_phase or np.random.rand() < explore:
            ...
            # Mine: Sampling {4} more actions
            extra_sampled_actions =[]
            for i in range(2): ## For MalConv 2 extra actions were used
                chosen_action_emb_ex, _ = self.actor.get_action(state, explore=0)
                action_ex = self.action_rep.get_best_match(chosen_action_emb_ex)
                extra_sampled_actions.append(action_ex)

        return action, chosen_action_emb, extra_sampled_actions
    ...
    def initial_phase_training(self, max_epochs=-1):
        ...
        for counter in range(max_epochs):
            ...
            if len(initial_losses) >= 20 and np.mean(initial_losses[-10:]) + 1e-6 >= np.mean(initial_losses[-20:]):
                print("Converged...")
                break
