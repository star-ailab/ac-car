...
class embed_DPG_basis(Agent):
    def __init__(self, config):
        super(embed_DPG_basis, self).__init__(config)
        ...
        self.action_rep = ActionRepresentation.Action_representation(action_dim=self.action_dim, state_dim=self.state_features.feature_dim, config=config)
    ...
    def optimize(self):
        ...
        # ---------------------- optimize action representations ----------------------
        # self.self_supervised_update(s1, a1, s2, self.config.emb_lambda)
    ...
    def self_supervised_training(self, eps=1e-3):
        ...
        while True:
            ...
            if idx > 1 and np.mean(losses[-3:]) - loss > eps:
                pass
            elif idx > 3:
                break
