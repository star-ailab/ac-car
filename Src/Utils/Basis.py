...
class Basis(NeuralNet):
    def __init__(self,  config):
        ...
        self.state_low = tensor(config.env.observation_space.min(), dtype=float32, requires_grad=False, device=config.device)
        self.state_high = tensor(config.env.observation_space.max(), dtype=float32, requires_grad=False, device=config.device)
...
class Basis_with_traces(NeuralNet_with_traces):
    def __init__(self,  config):
        ...
        ## mine: malware env's observation space is not Box ( does not have low or high) 
        else:             
            # Variables for normalizing state features
            self.state_low = tensor(config.env.observation_space.min(), dtype=float32, requires_grad=False, device=config.device)
            self.state_high = tensor(config.env.observation_space.max(), dtype=float32, requires_grad=False, device=config.device)
